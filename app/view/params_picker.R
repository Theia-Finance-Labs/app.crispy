...
server <- function(id, backend_trisk_params) {
  moduleServer(id, function(input, output, session) {
    # Store all inputs in a reactiveValues object
    trisk_params_selection <- reactiveValues()
    observe({
      trisk_params_selection$discount_rate <- input$discount_rate
      trisk_params_selection$risk_free_rate <- input$risk_free_rate
      trisk_params_selection$growth_rate <- input$growth_rate
      trisk_params_selection$shock_year <- input$shock_year
      trisk_params_selection$baseline_scenario <- input$baseline_scenario
      trisk_params_selection$shock_scenario <- input$shock_scenario
      trisk_params_selection$scenario_geography <- input$scenario_geography
    })

    update_dropdowns(input, session, backend_trisk_params)

    return(trisk_params_selection)
  })
}


update_dropdowns <- function(input, session, backend_trisk_params) {
  ...
  observe({
    ...
    new_choices <- unique(backend_trisk_params[backend_trisk_params$baseline_scenario == selected_baseline, ]$shock_scenario)
  ...
  observe({
    ...
    new_choices <- unique(backend_trisk_params |>
      dplyr::filter(
