box::use(
  ...
  app/logic/crispy_npv_change_plot[pipeline_crispy_npv_change_plot],
   app/logic/crispy_exposure_change_plot[pipeline_crispy_exposure_change_plot]
)
...
server <- function(id, analysis_data_r, max_trisk_granularity) {
  moduleServer(id, function(input, output, session) {
    observeEvent(analysis_data_r(), {
      ...
      exposure_change_plot <- pipeline_crispy_exposure_change_plot(analysis_data_r(), x_var = granul_top_level)
