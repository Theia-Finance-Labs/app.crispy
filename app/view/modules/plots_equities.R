...
server <- function(id, analysis_data_r, max_trisk_granularity) {
  moduleServer(id, function(input, output, session) {
    observeEvent(analysis_data_r(), {
      ...
      crispy_npv_change_plot <- trisk.plots::pipeline_crispy_npv_change_plot(analysis_data_r(), x_var = granul_top_level)
      output$crispy_npv_change_plot <- renderPlot({
        crispy_npv_change_plot
      })

      exposure_change_plot <- trisk.plots::pipeline_crispy_exposure_change_plot(analysis_data_r(), x_var = granul_top_level)
